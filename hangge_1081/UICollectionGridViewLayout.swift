...
class UICollectionGridViewLayout: UICollectionViewLayout {
    ...
    func sizeForItemWithColumnIndex(columnIndex: Int, remainingWidth: CGFloat) -> CGSize {
        ...
        let remainingWidth = collectionView!.frame.width -
            collectionView!.contentInset.left - collectionView!.contentInset.right
        
        let newWidth = Int(remainingWidth) / viewController.cols.count
        
        //每列等宽
        return CGSize(width:CGFloat(newWidth), height:size.height + 10)
